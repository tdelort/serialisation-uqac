...
int main()
{
    ...
    p.m_armure = 0;
    p.m_argent = 100;
    p.m_position = {10f, 0f, 0f};
    p.m_taille = {1f, 0.5f, 2f};
    p.m_rotation = {0.4964001f, 0.7791921f, 0.3227519f, -0.2056157f};

    std::pair<char*, int> buffer = Player.Write(p);

    Player p2 = Player.Read(buffer.first, buffer.second);
    
    std::cout << "vie : " << p.m_vie << std::endl;
    std::cout << "money : " << p.m_money << std::endl;
